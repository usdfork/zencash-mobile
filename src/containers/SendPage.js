...
class SendPage extends React.Component {
  constructor (props) {
    ...
    this.handleSendZEN = this.handleSendBZC.bind(this)
  ...
  // bzc and fiat and sets sendValue
      ...
      if (status.authorized) {
        ...
        QRScanner.scan(function (err, address) {
          ...
          if (err) {
            ...
            const scannedResultSeparated = address.split(/(?:bitzec:|=|&|\?)+/)
          ...
          .then((infoResp) => {
            ...
            axios.get(blockHashURL)
              .then((responseBhash) => {
                ...
                // fail and tell user
                if (satoshisSoFar < satoshisToSend + satoshisfeesToSend) {
                  alert(TRANSLATIONS[CUR_LANG].SendPage.notEnoughBZC)
                  this.setProgressValue(0)
                  return
                }
